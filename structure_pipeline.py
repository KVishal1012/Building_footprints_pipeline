...
class PipelineConfig:
    ...
    osm_overlap_ratio_threshold: float = 0.05
    parcel_overlap_ratio_threshold: float = 0.05
    boundary_sources: dict[str, dict] = field(default_factory=dict)
...
def source_for_city(
    ...
    city: str,
    state: str,
    provided_source: dict | str | None = None,
) -> dict | None:
    source = provided_source
    for key in (city_slug, slugify(city, state), slugify(city), f"{city}, {state}"):
        if source is None and key in sources:
            source = sources[key]
    if source is None:
        return None
...
def normalize_boundary(boundary: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    return clean_geom(boundary[["geometry"]]).reset_index(drop=True)


def read_boundary_source(source: dict) -> gpd.GeoDataFrame:
    if "path" not in source:
        raise ValueError("Boundary source must include a local 'path'")
    ...
    if boundary.crs is None:
        boundary = boundary.set_crs(source.get("crs", "EPSG:4326"))
...
def get_city_boundary(
    ...
    if boundary_source is not None:
        source = (
            ...
            else dict(boundary_source)
        )
        return read_boundary_source(source)

    place = f"{city}, {normalize_state_name(state)}, {country}"
    return normalize_boundary(ox.geocode_to_gdf(place))
...
def best_polygon_overlap(
    ...
    left = base[[base_id, "geometry"]].copy().reset_index(drop=True)
    right = candidates[candidate_columns + ["geometry"]].copy().reset_index(drop=True)
    left = clean_geom(left)
    right = clean_geom(right)
    if left.empty or right.empty:
        return pd.DataFrame()

    work_crs = estimated_projected_crs(left, right)
    left = left.to_crs(work_crs)
    right = right.to_crs(work_crs)
    left["_BaseArea_m2"] = left.geometry.area.replace(0, np.nan)
    right["_CandidateIndex"] = right.index

    joined = gpd.sjoin(
        left[[base_id, "_BaseArea_m2", "geometry"]],
        right[["_CandidateIndex", "geometry"]],
        how="inner",
    ...
    right_geometries = gpd.GeoSeries(
        right.geometry.iloc[joined["_CandidateIndex"].to_numpy()].to_numpy(),
        index=joined.index,
        crs=work_crs,
    )
    joined["_OverlapArea_m2"] = joined.geometry.intersection(right_geometries).area
    joined["_OverlapRatio"] = joined["_OverlapArea_m2"] / joined["_BaseArea_m2"]
    joined = joined[
        (joined["_OverlapArea_m2"] > 0)
        & (joined["_OverlapRatio"].fillna(0) >= min_overlap_ratio)
    ]
    if joined.empty:
        return pd.DataFrame()

    best = (
        joined.sort_values([base_id, "_OverlapArea_m2"], ascending=[True, False])
        .drop_duplicates(base_id)
        .reset_index(drop=True)
    )
    attributes = right.drop(columns="geometry").iloc[
        best["_CandidateIndex"].to_numpy()
    ]
    attributes = attributes.reset_index(drop=True)
    attributes[base_id] = best[base_id].to_numpy()
...
def read_parcel_file(source: dict, boundary: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    path = Path(source["path"]).expanduser()
    if not path.exists():
        print(f"Parcels skipped: file not found at {path}")
        return empty_gdf()
    ...
    if parcels.crs is None:
        ...
        print(f"Parcels file has no CRS; assuming {source_crs}")
        parcels = parcels.set_crs(source_crs)
    return clip_to_boundary(parcels, boundary)


def read_arcgis_parcels(source: dict, boundary: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    ...
    while True:
        ...
        try:
            ...
            print(f"Parcels skipped: ArcGIS request failed for {url}: {exc}")
            return empty_gdf()
...
def standardize_parcels(
    ...
    projected_crs = out.estimate_utm_crs() or "EPSG:5070"
...
def load_city_parcels(
    ...
    if source.get("path"):
        raw = read_parcel_file(source, boundary)
    elif source.get("url"):
        raw = read_arcgis_parcels(source, boundary)
    else:
        print("Parcels skipped: source must include either 'path' or 'url'")
        return empty_gdf()
...
def parse_place(value: str) -> dict[str, str]:
    if "," not in value:
        raise argparse.ArgumentTypeError(
            "Use 'City, State', for example 'Houston, Texas'"
        )
...
def parse_parcel_source_arg(value: str) -> tuple[str, dict]:
    if "=" not in value:
        raise argparse.ArgumentTypeError(
            "Use city_slug=path_or_url, for example houston_texas_usa=data/parcels.gpkg"
        )
